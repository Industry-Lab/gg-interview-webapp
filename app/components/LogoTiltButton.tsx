...
interface LogoTiltButtonProps {
  onClick?: () => void;
  className?: string;
  alt?: string;
}

const LogoTiltButton: React.FC<LogoTiltButtonProps> = ({ onClick, className = '', alt = 'Logo Button' }) => {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setTiltStyle({
      ...
      boxShadow: `0 15px 35px rgba(0, 255, 204, 0.2), 0 3px 10px rgba(0, 0, 0, 0.2), ${rotateY / 3}px ${rotateX / 3}px 10px rgba(0, 0, 0, 0.1)`
    });
  };
  ...
  return (
    <button
      ref={buttonRef}
      className={`rounded-xl overflow-hidden shadow-xl bg-black/60 transition-transform duration-200 ${className}`}
      style={tiltStyle}
